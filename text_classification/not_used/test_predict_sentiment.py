...
def read_data(target_name):
    X = []
    Y = []
    with open("new_dataset/sentiment_data_" + target_name + ".txt", "r") as f:
        for one_data in f:
...
def get_preprocessed_data(target_name):
    ...
    Y = tensorflow.keras.utils.to_categorical(Y, num_classes=num_labels)
...
def build_fastText_model(max_features):
    model = Sequential()
    model.add(Embedding(input_dim=max_features, output_dim=8, input_length=500))
    model.add(LSTM(128))
